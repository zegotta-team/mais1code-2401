...
abstract class VagaDTO implements DTOInterface
{
    ...
    public static function salvar($vaga)
    {
        ...
        if (empty($vaga->getId())) {
            ...
            VALUES ({$vaga->getEmpresa()->getId()}, \"{$vaga->getTitulo()}\", \"{$vaga->getEmail()}\", {$vaga->getSalario()}, \"{$vaga->getBeneficios()}\", \"{$vaga->getDescricao()}\", \"{$vaga->getRequisitos()}\",\"{$vaga->getCargaHoraria()}\", {$vaga->getCargaHoraria()})";
        } else {
            $sql = "UPDATE vaga SET ";
